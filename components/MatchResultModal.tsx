...
interface MatchResultModalProps {
  match: Omit<Match, 'player1Id' | 'player2Id'> & { player1Id: number; player2Id: number; }; // Ensure players are not null
  player1: Player;
  player2: Player;
  onClose: () => void;
  onSave: (match: Match, newWinnerId: number, newScore: string, newIsWO: boolean) => void;
  isBracketMatch?: boolean;
}

export const MatchResultModal: React.FC<MatchResultModalProps> = ({ match, player1, player2, onClose, onSave, isBracketMatch = false }) => {
  const [winnerId, setWinnerId] = useState<number | null>(match.winnerId);
  const [isWO, setIsWO] = useState(match.isWO);
